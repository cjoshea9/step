...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void testOneOptionalSameResult() {
    // Have each person have different events. We should see three options because each person has
    // split the restricted times even though there's an optional over the whole day.
    //
    // Events  :       |--A--|     |--B--|
    // Optional: |--------------C--------------|
    // Day     : |-----------------------------|
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 2", TimeRange.fromStartDuration(TIME_0900AM, DURATION_30_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartDuration(0, TimeRange.WHOLE_DAY),
  ...
  @Test
  public void testOneOptionalDifferentTimes() {
  ...
  public void justEnoughRoomWithOptional() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting despite the optional guest.
    //
    // Events  : |--A--|        |----A----|
    // Optional:       |-B-|  
    // Day     : |------------------------|
  ...
  @Test
  public void twoOptionalWithGaps() {
    // Just optional guests who have space in their calendars for meetings.
    //
    // Optional: |---A---|     |---B------|
    // Day     : |------------------------|
    // Options :         |-----|
    ...
    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected =
        Arrays.asList(TimeRange.fromStartDuration(TIME_0830AM, DURATION_90_MINUTES));

    Assert.assertEquals(expected, actual);
  }

  @Test
  public void twoOptionalWithoutGaps() {
    // Just optional guests with no space in calendar.
    //
    // Optional: |---A-----|---B------|
    // Day     : |--------------------|
    // Options :         
